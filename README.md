...
On first start, Docker creates `./dslflow_data` if it does not exist and the entrypoint fixes its ownership. Custom nodes are already in the image — no seeding or `npm install` step is needed. Node-RED will prompt you to create or clone a project.
